...
describe('docker compose supervisor env wiring', () => {
  ...
  it('keeps the sandbox runtime package version centralized in the Docker version file', async () => {
    ...
    expect(versionFile).toContain('SANDBOX_RUNTIME_NPM_VERSION=0.5.2');
    expect(composeFile).toContain(
      'SANDBOX_RUNTIME_NPM_VERSION: ${SANDBOX_RUNTIME_NPM_VERSION:-}',
    );
    expect(composeFile).not.toContain('SANDBOX_RUNTIME_NPM_VERSION:-0.5.2');
    ...
    expect(dockerfile).not.toContain('ARG SANDBOX_RUNTIME_NPM_VERSION=0.5.2');
    expect(envExample).toContain('# SANDBOX_RUNTIME_NPM_VERSION=');
    expect(envExample).not.toContain('# SANDBOX_RUNTIME_NPM_VERSION=0.5.2');
